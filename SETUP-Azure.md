...
* [Provide the platform with Azure credentials](https://www.serverless.com/framework/docs/providers/azure/guide/credentials)
***Note - please make sure the service principle has enough permission to create resource group, create functions, etc...***
...
```bash
mv serverless.azure.example.yml serverless.yml (or platform of choice)
npm install
```

### Step 5- Install cloud platform specific dependencies, and prepare for configuration
```bash
cd azure (or platform of choice)
npm install
```
...
```
### Step 8.5 (Before deploy)- Insert required property to serverless.yml
The serverless framework will create a storage account with the certain name. Azure requires the storage name to be gloabally unique. You need to insert the storageAccount property into the provider section in the serverless.yml file to manually define the storage account name.
```yaml
  provider:
  name: azure
  runtime: nodejs12
  ...
  os: linux
  storageAccount:
    name: <storage account name, this should be globally unique>
```
...
At this point you should have a number of serverless functions in Azure. Continue on to setup the rest of the assets in Okta to support this reference implementation.
...
Note: the value you'll use is the URL for your API Gateway URL + tokenhook. Example: `https://{uid}.azurewebsites.net/api/smartendpoints/tokenhook`
...
The reference implementation comes with example resource server endpoints- so if you're using this implementation for example/reference purposes, this value will take the form: https://xxxyyy.azurewebsites.net/dev
